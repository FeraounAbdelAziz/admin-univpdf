...
export default function SpecialtyLayout({
    ...
    useEffect(() => {
        ...
        const fetchPaths = async () => {
            ...
            let { data: specialty, error } = await supabase.from('specialty')
            ...
            if (specialty) {
                ...
                if (param.module) {
                    let { data, error: error_2 } = await supabase.from('module')
                        .select('module_name')
                        .eq('module_id', param.module)
                        .single()

                    if (data) {
                        setItems((items) => [...items, { title: data.module_name, href: param.module as string }])
                    }

                } else {
                    setItems((items) => [ { title: specialty.specialty_name, href: "/dashboard/" + param.specialty as string }])
